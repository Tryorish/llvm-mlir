...
struct SimplifyRedundantNeg : public mlir::OpRewritePattern<NegOp> {
  ...
  llvm::LogicalResult
  matchAndRewrite(NegOp op, mlir::PatternRewriter &rewriter) const override {
    mlir::Value input = op.getOperand();
    NegOp inputOp = input.getDefiningOp<NegOp>();
...
// 注册为 NegOp 的 canonicalization pattern，使得它们可以被 Canonicalization framework 选中。
