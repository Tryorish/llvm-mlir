...
llvm::LogicalResult MatMulOp::verify() {
  ...
  if (lhsType.getRank() != 2 || rhsType.getRank() != 2)
    return emitOpError() << "expects lhs and rhs to be rank-2 tensors";

  if (lhsType.getShape()[1] != rhsType.getShape()[0])
    return emitOpError() << "expects lhs shape MxK and rhs shape KxN";

  if (resultType.getRank() != 2)
    return emitOpError() << "expects result to be a rank-2 tensor";

  if (resultType.getShape()[0] != lhsType.getShape()[0] ||
      resultType.getShape()[1] != rhsType.getShape()[1])
    return emitOpError() << "expects result shape to be MxN";
...
/// Infer the output shape of the NegOp, this is required by the shape inference
/// interface.
void NegOp::inferShapes() { getResult().setType(getInput().getType());}
